...
public class Friend {
    ...
    public static ArrayList<UUID> getOnlineFriends(UUID user) {
        ArrayList<UUID> list = new ArrayList<>();
        try {
            ResultSet rs = MySQL.querry("SELECT other FROM friend, user WHERE friend.user = '" + user + "' AND friend.other = user.uuid AND accepted = " + true + " AND user.lastseen = -1");
            while (rs.next()) {
                list.add(UUID.fromString(rs.getString("other")));
            }
        } catch (SQLException e) {
